...
from tensorflow.keras.layers import Activation, Conv2D, Input, LeakyReLU  # pylint: disable=import-error
...
def main(_):
    ...
    net = run(job_dir=flags.FLAGS["job-dir"].value,
              dataset=dataset,
              epochs=flags.FLAGS.epochs,
              steps_per_epoch=flags.FLAGS.steps_per_epoch,
              learning_rate=flags.FLAGS.learning_rate)

    tf.keras.experimental.export_saved_model(net,
                                             "%s/export/discriminator" %
